    ...
    return (
        <div className={classes.root}>
            <AppBar position="fixed" color="default">
                <Toolbar>
                    <IconButton edge="start" className={classes.logo} color="inherit" aria-label="menu" onClick={()=>setLoggedIn(!loggedIn)}>
                        <img src={logo} width="100%"/>
                    </IconButton>
                    <Typography variant="h6" className={classes.title}>{loggedIn ? "Afluence" : ' '}</Typography>
                    {!loggedIn && <><IconButton onClick={handleMenuClick} color="inherit">
                        <AccountCircleIcon />
                    </IconButton>
